...
impl DirectXEffects {
    ...
    pub fn end_layer(&mut self, frame: &EffectFrame, layer: &EffectLayer) -> Result<()> {
        ...
        let blurred_shadow = if layer.has_shadow != 0 && shadow_sigma > 0.0 {
            Some(self.blur(frame, &content, region, shadow_sigma)?)
        } else {
            None
        };
...
/// Clears t0 and t2 to t4 so a texture can become a render target again.
...
mod tests {
    ...
    fn params_match_the_shader_layout() {
        ...
        assert_eq!(std::mem::size_of::<LayerComposite>(), 656);
    }
}

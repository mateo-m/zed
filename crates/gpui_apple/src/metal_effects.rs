...
impl Effects {
    ...
    pub fn end_layer<'a>(
        ...
        let blurred_under = (layer.backdrop_blur > 0.0).then(|| {
            if layer.has_mask != 0 {
                self.variable_blur(device, frame, &under, region, &composite)
            } else {
                self.blur(
                    device,
                    frame,
    ...
    fn variable_blur(
        ...
        let first = self.take_texture(device, size, MTLPixelFormat::BGRA8Unorm);
        ...
        let second = self.take_texture(device, size, MTLPixelFormat::BGRA8Unorm);
        self.variable_blur_pass(
            frame,
            &first,
            &second,
